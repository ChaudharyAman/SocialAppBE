  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "username, first_name, last_name, email, password_hash, bio, date_of_birth, gender, mobile, account_status is required",
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "oldUsername, username, first_name, last_name, email, password, bio, date_of_birth, gender, mobile, account_status is required",
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "username, first_name, last_name, email, password, bio, date_of_birth, gender, mobile, account_status are required",
      });
    }
    ...
    if (!moment(formattedDOB, "YYYY-MM-DD", true).isValid()) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid date_of_birth format. Use YYYY-MM-DD, YYYY/MM/DD, or DD-MM-YYYY",
      });
    }
...
exports.getUserByUsername = async (req, res) => {
  try {
    ...
    const users = await User.findAll({
      ...
      order: [[fn("LENGTH", col("username")), "ASC"]],
    });
