...
exports.sendMessage = async (req, res) => {
  ...
  if (!friendId || !message) {
    return res
      .status(400)
      .json({ success: false, message: "friend_id and message are required" });
  }
  ...
  try {
    ...
    if (!isFriend) {
      return res
        .status(403)
        .json({ success: false, message: "You can only message your friend" });
    }
      ...
      include: [
        {
          model: User,
          as: "sender",
          attributes: ["id", "username", "media_url"],
        },
        {
          model: User,
          as: "receiver",
